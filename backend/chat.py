...
async def start_game(novel_id: str, chapter_id: str, player_identity: PlayerIdentity,
    ...
    # Check if memory data exists for multi-agent pipeline
    memory_manager = MemoryManager(novel_id)
    use_multi_agent = memory_manager.has_memory_data()

    # Compute spatiotemporal coordinate
    scene_coord_str = ""
    if use_multi_agent and scene_data:
        scene_id_val = scene_data.get("id", "")
        coord = TimeCoord.from_scene_id(novel_id, chapter_id, scene_id_val)
        scene_coord_str = coord.to_scene_coord().to_key()
        memory_manager.initialize()
        memory_manager.start_scene(scene_coord_str)

    # Initialize Game Master (for opening narrative and fallback)
...
async def game_chat(session_id: str, message: str) -> list[GameMessage]:
    ...
    # Check if multi-agent pipeline is available
    t0 = time.time()
    memory_manager = MemoryManager(session.novel_id)
    use_multi_agent = memory_manager.has_memory_data()
    print(f"[chat] MemoryManager init + has_memory_data: {(time.time()-t0)*1000:.0f}ms, use_multi_agent={use_multi_agent}")

    # Initialize world
    t0 = time.time()
    world = WorldState(session.novel_id, session.chapter_id)
    player_name = _get_player_name(session.player_identity, world)
    print(f"[chat] WorldState init: {(time.time()-t0)*1000:.0f}ms")
    ...
    scene_data = scenes[0] if scenes else {}

    if use_multi_agent:
        # === Multi-agent pipeline ===
        t0 = time.time()
        memory_manager.initialize()
        print(f"[chat] memory_manager.initialize(): {(time.time()-t0)*1000:.0f}ms")

        scene_id_val = scene_data.get("id", "")
        coord = TimeCoord.from_scene_id(session.novel_id, session.chapter_id, scene_id_val)
        scene_coord = coord.to_scene_coord().to_key()
        memory_manager.start_scene(scene_coord)

        from backend.multi_agent import process_with_multi_agent
        response_messages = await process_with_multi_agent(
        ...
        # === Legacy GameMaster pipeline (backward compatible) ===
        ...
        if session.player_identity.mode == IdentityMode.POSSESS:
            char_state = world.get_character_state(player_name)
        else:
        ...
        print(f"[chat] CognitiveValidator.validate(): {(time.time()-t0)*1000:.0f}ms")

        t0 = time.time()
        response_messages = await gm.process_player_action(message, validation)
        print(f"[chat] gm.process_player_action(): {(time.time()-t0)*1000:.0f}ms")
    ...
    t0 = time.time()
    _save_session(session)
    print(f"[chat] _save_session(): {(time.time()-t0)*1000:.0f}ms")

    print(f"[chat] === game_chat TOTAL: {(time.time()-t_total)*1000:.0f}ms ===")
