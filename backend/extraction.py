...
async def _llm_call(prompt: str, system_prompt: str = "") -> str:
    ...
    # 直接用 OpenAI 客户端，绕过 lightrag 的重试装饰器，精确测量单次请求耗时
    client = AsyncOpenAI(api_key=LLM_API_KEY, base_url=LLM_BASE_URL, timeout=120)
    messages = []
    if system_prompt:
        messages.append({"role": "system", "content": system_prompt or "你是一个小说分析助手。"})
    ...
    try:
        ...
        if usage:
            print(f"[llm_call] {http_ms:.0f}ms | prompt={prompt_len}chars sys={sys_len}chars | "
                  f"tokens: in={usage.prompt_tokens} out={usage.completion_tokens} | "
                  f"resp={len(result)}chars")
        else:
            print(f"[llm_call] {http_ms:.0f}ms | prompt={prompt_len}chars sys={sys_len}chars | resp={len(result)}chars")

    except Exception as e:
        http_ms = (time.time() - t0) * 1000
        print(f"[llm_call] FAILED after {http_ms:.0f}ms | error={type(e).__name__}: {e}")
        raise

    total_ms = (time.time() - t_total) * 1000
    if total_ms - http_ms > 100:
        print(f"[llm_call] overhead: {total_ms - http_ms:.0f}ms (client init, etc.)")
...
async def extract_novel(file_path: str, novel_id: str) -> dict:
    ...
    # Step 8: Migrate to 4-layer memory system
    try:
        from backend.migrate_memory import migrate_novel_to_memory
        memory_stats = await migrate_novel_to_memory(novel_id)
        print(f"Memory migration: {memory_stats.get('total_fragments', 0)} fragments")
    except Exception as e:
        print(f"Memory migration failed (non-fatal): {e}")
